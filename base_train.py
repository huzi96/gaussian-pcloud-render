...
from pointersect_utils import print_and_save
from pointersect_utils.argparse_utils import WarnDict as dict
from pointersect_utils.argparse_utils import print_options
from pointersect_utils.argparse_utils import read_config_file
from pointersect_utils.argparse_utils import recursive_dict_update
from pointersect_utils.print_and_save import Logger
from pointersect_utils.print_and_save import StatisticsCollector
...
class BaseTrainProcess(ABC):
    ...
    def run(self):
        ...
        if self.process_info["distributed_run"]:
            raise NotImplementedError("Distribution run is not supported yet.")
        ...
        self.stats = os.path.join(dir_dict["stats"], "result.csv")
        #bpp t hit normal cd
        ...
        while self.epoch <= self.process_info["end_epoch"]:
            ...
            # save test result flag
            if self.epoch % self.process_info["save_every_num_epoch"] == 0 and self.process_info["rank"] == 0:
                sdm = out_dict["stats_dict"]["mean"]
                stat_f = open(self.stats,"a+")
                if self.epoch == 0:
                    self.stat_keys = list(sdm.keys())
                    stat_f.write(",".join(self.stat_keys)+"\n")
                epoch_res = [str(sdm[key]) for key in self.stat_keys]  
                stat_f.write(",".join(epoch_res)+"\n")
                stat_f.close()
            if self.process_info["distributed_run"]:
                torch.distributed.barrier()
    ...
    def save(self, filename: str, epoch: int = None):
        ...
        try:
            schedulers = inspect.getmembers(self, lambda v: isinstance(v, torch.optim.lr_scheduler._LRScheduler))  # name, scheduler
            for name, scheduler in schedulers:
                to_save[name] = scheduler.state_dict()
        except:
            pass
