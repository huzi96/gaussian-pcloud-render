...
import pointersect_utils.meta_script_utils as meta_utils
...
def get_dataset_mesh_filename() -> str:
    filename = './config/mesh_filenames.yaml'
    return filename
...
def gather_and_clean_dataset(
    ...
    if rank == 0:
        ...
        if dataset_name.lower() in {
                ...
                'tex-models','thuman-test','thuman-debug',
                'longdress-test', 'thuman-full', 'thuman-max',
                'thuman-pro', 'thuman-cross-subject', 'thuman-cs-test',
                'thuman-cs-valid', 'thuman-cross-subject-small',
                'blended-mvs-test', 'blended-mvs-standalone',
                'mvs_thu_mix_nonvox_v1', 'mvs_thu_mix_nonvox_dryrun'
        }:
...
def load_mesh_and_pcd_filename(
        mesh_filenames: T.Union[str, T.List[str]],
        mesh_scale: float = 1.,
        printout: bool = True,
        num_pcd: int = 1,
    ...
    for i in range(len(mesh_filenames)):
        ...
        if num_pcd == 1:
            pcd_name = mesh_filename[:-8]+"pcd_0.ply"

            pcd = o3d.io.read_point_cloud(pcd_name)
            # kdtree = o3d.geometry.KDTreeFlann(pcd)
            # kd_trees.append(kdtree)
            # pcd.estimate_normals()
            # pcd.orient_normals_consistent_tangent_plane(100)
            pcd = structures.PointCloud.from_o3d_pcd(pcd)
            pcds.append(pcd)
        else:
            pcd_list = []
            for j in range(num_pcd):
                pcd_name = mesh_filename[:-8]+"pcd_"+str(j)+".ply"
                pcd = o3d.io.read_point_cloud(pcd_name)
                # pcd.estimate_normals()
                # pcd.orient_normals_consistent_tangent_plane(100)
                pcd = structures.PointCloud.from_o3d_pcd(pcd)
                pcd_list.append(pcd)
            pcds.append(tuple(pcd_list))

    return meshes, pcds

def load_pcd_filename(
        mesh_filenames: T.Union[str, T.List[str]],
        printout: bool = True,
) -> T.List[structures.Mesh]:
    """
    ...
    for i in range(len(mesh_filenames)):
        ...
        # In default, clean_mesh is true, note that even bunny should be cleaned to multiply Kd and texture map
        if printout:
            print(f'Loading {i}/{len(mesh_filenames)} pcd: {mesh_filename}...', flush=True)
...
def get_dataset(
    ...
    if dataset_info.get('pcd_only', False):
        raise NotImplementedError('Point cloud only dataset is not yet implemented.')
    else:
        if dataset_info.get('mix_meshes', False):
            ...
            meshes, pcds = load_mesh_and_pcd_filename(
                mesh_filenames=train_mesh_filenames,
                mesh_scale=dataset_info.get('mesh_scale', 1.),
                printout=printout,
                num_pcd=num_pcd,
        ...
        for i in range(len(meshes)):
            ...
            if mode == 'uniform':
                if num_pcd == 1:
                    datasets.append(
                        mesh_dataset_v2.MeshDataset(
                            ...
                            rng_seed=dataset_info.get('dataset_rng_seed', None) + 13 * i,
                    ...
                    datasets.append(
                        mesh_dataset_v2.MeshDatasetMultiPCD(
                            mesh=mesh,
                            pcd = pcds[i],
                            kd_tree = None,
                            n_target_imgs=dataset_info['n_target_imgs'],
                ...
                datasets.append(
                    mesh_dataset_v2.MeshRGBDDataset(
                        mesh=mesh,
                        pcd = pcds[i],
                        kd_tree = None,
                        n_target_imgs=dataset_info['n_target_imgs'],
                        ...
                        min_subsample=dataset_info.get('min_subsample', 1),
                        max_subsample=dataset_info.get('max_subsample', 1),
                        min_rnd_num_points=dataset_info.get('min_rnd_num_points', 100000),
                        max_rnd_num_points=dataset_info.get('max_rnd_num_points', 1000000),
                        perturb_camera=dataset_info.get('perturb_camera', False),
                        fov=dataset_info.get('fov', 60),
                        perturb_camera_origin_rate=dataset_info.get('perturb_camera_origin_rate', 0.),
            ...
            # validation datasets are one of the meshes but different views
            if i == 13 or i == 0:
                if mode == 'uniform':
                    if num_pcd == 1:
                        val_datasets.append(
        ...
        if test_mesh_filenames is None or len(test_mesh_filenames) == 0:
            ...
            if dataset_info.get('mix_meshes', False):
                # combine multiple meshes
                test_meshes: T.List[structures.Mesh] = load_and_mix_mesh_filename(
                    mesh_filenames=test_mesh_filenames,
                    mesh_scale=dataset_info.get('mesh_scale', 1.),
                    min_num_mesh=dataset_info.get('min_num_mesh', 1),
                    max_num_mesh=dataset_info.get('max_num_mesh', 2),
                ...
                test_meshes, test_pcds = load_mesh_and_pcd_filename(
                    mesh_filenames=test_mesh_filenames,
                    mesh_scale=dataset_info.get('mesh_scale', 1.),
                    printout=printout,
                    num_pcd=num_pcd,
            ...
            output_camera_trajectory_params['output_cam_mode'] = 'spiral'
            for i in range(len(test_meshes)):
                if printout:
                    print(f'creating test dataset: {i}/{len(test_meshes)}', flush=True)
                if mode == 'uniform':
                    if num_pcd == 1:
                        test_datasets.append(
                            mesh_dataset_v2.MeshDataset(
                                ...
                                pcd=test_pcds[i],
                                kd_tree=None,
                                n_target_imgs=dataset_info['n_test_target_imgs'],
                                n_imgs=dataset_info['n_imgs'],
